...
!/ *** mise en commentaire du module type car pas utilise***/ scl 04-12-2018
      ...
      ! interface verif1
      ! subroutine marq98j_scl2(k0,b,m,ni,v,rl,ier,istop,effet,ca,cb,dd,fctnames,I_hess,&
                           ! H_hess,hess,vvv)
     ...
     ! double precision::fctnames
      ...
      ! end subroutine marq98j_scl2

      ! subroutine derivaj_scl(b,m,v,rl,k0,fctnames)
        ! !double precision,dimension(:,:),intent(inout)::wij_chap
        ! integer,intent(in)::m
    ! double precision,dimension(2)::k0
    ...
    ! double precision,external::fctnames
      ...
      ! subroutine searpasj_scl(vw,step,b,bh,m,delta,fim,k0,fctnames)
        ! !double precision,dimension(:,:),intent(inout)::wij_chap
        ! integer,intent(in)::m
    ! double precision,dimension(2)::k0
        ! double precision,dimension(m),intent(in)::b
    ...
    ! subroutine valfpaj_scl(vw,fi,b,bk,m,delta,k0,fctnames)
        ! !double precision,dimension(:,:),intent(inout)::wij_chap
        ! integer,intent(in)::m
        ! double precision,dimension(2)::k0
...
! -Interface permettant la verification des type des arguments
      interface verif1
        module procedure marq98j_scl2,derivaj_scl,searpasj_scl,dmfsdj,valfpaj_scl
      end interface verif1
...
!wij_chap: contient les valeur estimees de w_ij_chapeau cas specifique a l'estimation pealable au niveau individuel des effets
                           !
...
!---------- ajout
    ...
    ! if(model==10) then
        ...
        ! stop
    ! endif
    ! !print*,"suis la 0",size(wij_chap),size(wij_chap,1),size(wij_chap,2)
    ! stop
...
!        !print*,b
! !print*,"suis la 011",size(wij_chap),size(wij_chap,1),size(wij_chap,2)
        call derivaj_scl(b,m,v,rl,k0,fctnames)
! !print*,"suis la 1",size(wij_chap),size(wij_chap,1),size(wij_chap,2)
! stop
    rl1=rl
    if(rl.eq.-1.D9) then
        istop=4
...
!      !write(6,*) 'loglikelihood not improved '
        ...
        step=dlog(1.5d0)
!      !write(*,*) 'searpas'
        call searpasj_scl(vw,step,b,bh,m,delta,fi,k0,fctnames)
        rl=-fi
        if(rl.eq.-1.D9) then
            istop=4
            goto 110
...
! fin ajout amadou
    ...
    ep=10.d-10
    call derivaj_scl(b,m,vnonpen,rl,zero,fctnames)
...
!------------------------------------------------------------
    ...
    double precision::thn,th,z,vl,th2,vaux
    double precision,external::fctnames
...
!
    ...
    !double precision,dimension(:,:),intent(inout)::wij_chap
      ...
      integer::i
       ...
       vlw1=dlog(vw)
       vlw2=vlw1+step
       call valfpaj_scl(vlw1,fi1,b,bh,m,delta,k0,fctnames)
       call valfpaj_scl(vlw2,fi2,b,bh,m,delta,k0,fctnames)
...
!
      ...
      call valfpaj_scl(vm,fim,b,bh,m,delta,k0,fctnames)    
