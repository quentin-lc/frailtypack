...
subroutine joint_longi(VectNsujet,ngnzag,k0,tt00,tt10,ic0,groupe0&
        ...
        ,paratps,BetaMatTps,EPS,GH,paGH,med,varcenters,&
        vartreatment,param_res_rt,res_rt)
    ...
    use lois_normales
    use optim
    use var_surrogate, only:Chol,a_deja_simul,Vect_sim_MC,graine,mediation
    use var_mediation,only:treat_ind,center_ind,nparammed,&
                           ncenters,nmcmed,niter,nmcfrail,rank_trtM,rank_trtT,&
                           rank_trtMt,rank_time,&
                           method_int,type_ma,matmc,matmc_frail,&
                           QMC1,QMC2,rt_nparams,rt_sizes
    !use Autres_fonctions, only:bgos, uniran
    use donnees,only:QMC1000_1,QMC1000_2,QMC500_1,QMC500_2!,!QMC5000_1,QMC5000_2,
    ! add Monte-carlo
    ...
    integer,dimension(6),intent(in)::params_hazard 
    integer,dimension(4)::mtaille 
    integer::equidistant 
    integer::typeof0
    !param_hazard(1) = typeof, (2) = equidistant, (3:6) = mtaille 
    ...
    double precision,dimension(params_hazard(3))::x1Out
    double precision,dimension(params_hazard(4))::x2Out
    double precision,dimension(params_hazard(3),3)::lamOut
    double precision,dimension(params_hazard(5),3)::suOut
    double precision,dimension(params_hazard(4),3)::lam2Out
    double precision,dimension(params_hazard(6),3)::su2Out
    ...
    double precision,external::funcpajLongisplines,funcpajLongiweib,funcpajLongisplines2
    ...
    integer::ngtemp
    integer,dimension(3,2),intent(in)::paratps
    !integer,dimension(VectNvar(1)+VectNvar(2)+VectNvar(3)),intent(in)::filtretps0
    !important: to reduce number of arguments, previous two (vector) arguments
    ! 'paratps' and 'filtretps0' have been merged into a single (matrix) argument
    ! 'paratps'. The second column of this new argument correspond to the previous
    ! filtretps0.
    double precision,dimension(0:100,0,3)::BetaMatTps
    double precision,dimension(0:100,0:4*sum(paratps(1:VectNvar(1),2)))::BetaTpsMat !!! a refaire
    double precision,dimension(0:100,0:4*sum(paratps(VectNvar(1)+1:VectNvar(1)+VectNvar(2),2)))::BetaTpsMatDc
    double precision,dimension(0:100,0:4*sum( &
        paratps(VectNvar(1)+VectNvar(2)+1:VectNvar(1)+VectNvar(2)+VectNvar(3),2)))::BetaTpsMatY
    double precision,dimension(paratps(2,1)+paratps(3,1))::basis
    ...
    double precision,dimension(9)::med
    integer,dimension(ngnzag(1))::varcenters,vartreatment
    double precision,dimension(:),allocatable::breduced
    integer::reduced 
    integer,dimension(4),intent(in)::param_res_rt
    !param_res_rt(1) : rt.ntimes 
    !            (2) : rt.nmc
    !            (3) : rt.boot
    !            (4) : rt.nboot
    double precision,dimension(param_res_rt(1),4,1+param_res_rt(3)*&
                               param_res_rt(4)),intent(out)::res_rt

    typeof0 = params_hazard(1)
    equidistant = params_hazard(2)
    mtaille = params_hazard(3:6)
    !double precision::dummy
    BetaMatTps = 0.d0 !for now BetaMatTps is unused, 
    !this instruction is simply there to prevent gfortran warnings for unused variables
    ...
    timedep = paratps(1,1)
    nbinnerknots = paratps(2,1)
    qorder = paratps(3,1)
    allocate(filtretps(nva10),filtre2tps(nva20),filtre3tps(nva30))
    allocate(betatps(nva10),betatps2(nva20),betatps3(nva30))
    filtretps = paratps(1:nva10,2)
    filtre2tps = paratps((nva10+1):(nva10+nva20),2)
    filtre3tps = paratps((nva10+nva20+1):(nva10+nva20+nva30),2)
    ...
    if(med(1).eq.1)then
      ...
      rank_trtM=INT(med(4))
      rank_trtT=INT(med(5))
      rank_trtMt=INT(med(6))
      rank_time=INT(med(7))
      !method_int=INT(med(6))
      nmcmed=INT(med(8))
      !type_ma=INT(med(9))
      nparammed=INT(med(9))
      !timetype=INT(med(11))
      !Cmult=INT(med(12))
      !binit1=dble(med(10:(10+np-1)))
      allocate(treat_ind(ng),center_ind(ng))
      do i=1,ng
        treat_ind(i)=vartreatment(i)
      end do
      do i=1,ng
        center_ind(i)=varcenters(i)
      end do
      nmcfrail = 10000
      allocate(QMC1(nmcfrail),QMC2(nmcfrail))
      nmcmed = 500
      allocate(matmc(nmcmed,2))
      allocate(matmc_frail(nmcfrail,2))
      do i=1,nmcmed
        if(nmcmed.eq.500) then
          matmc(i,:)=(/QMC500_1(i),QMC500_2(i)/)
        else if(nmcmed.eq.1000) then 
          matmc(i,:)=(/QMC1000_1(i),QMC1000_2(i)/) 
        else if(nmcmed.eq.5000) then 
          !matmc(i,:)=(/QMC5000_1(i),QMC5000_2(i)/) 
        else if(nmcmed.eq.10) then 
          !matmc(i,:)=(/QMC5000_1(i),QMC5000_2(i)/) 
        else if(nmcmed.eq.100) then 
          !matmc(i,:)=(/QMC5000_1(i),QMC5000_2(i)/) 
        end if 
      enddo 
      do i=1,nmcfrail 
        if(nmcfrail.eq.500) then
          matmc_frail(i,:)=(/QMC500_1(i),QMC500_2(i)/)
        else if(nmcfrail.eq.1000) then 
          matmc_frail(i,:)=(/QMC1000_1(i),QMC1000_2(i)/) 
        else if(nmcfrail.eq.5000) then 
          !matmc_frail(i,:)=(/QMC5000_1(i),QMC5000_2(i)/) 
        else if(nmcfrail.eq.10) then 
          !matmc_frail(i,:)=(/QMC5000_1(i),QMC5000_2(i)/) 
        else if(nmcfrail.eq.100) then 
          !matmc_frail(i,:)=(/QMC5000_1(i),QMC5000_2(i)/) 
        end if 
      end do 
    else
    ...
    !allocate(nmesrec(ng),nmesrec1(ng),nmesy(ng))
    ...
    if(typeof == 0) then
      allocate(mm3(ndatemax),mm2(ndatemax),mm1(ndatemax),mm(ndatemax),&
               im3(ndatemax),im2(ndatemax),im1(ndatemax),im(ndatemax))
    end if
    !!!  DONNEES DECES
    aux = 0.d0
    mindc = 0.d0
    ...
    if(typeJoint.ge.2) then
      ...
      case(0)
        ...
        !                         else
        niter=0
        !b((np-2):np) = (/0.5d0,0.5d0,0.5d0 /)
        !call marq98J(k0,b,np,ni,v,res,ier,istop,effet,ca,cb,dd,funcpajlongisplines)
        if(med(1).eq.1) then 
          if(ncenters.ne.0) then
            if(reduced.eq.1) then 
              allocate(breduced(np-3))
              do i =1,np-3
                breduced(i)=0.5d0
              end do 
              nodes_number=9
              call marq98J(k0,breduced,np-3,ni,v,res,ier,istop,effet,ca,cb,dd,funcpajlongisplines)
              method_GH=4
              call marq98J(k0,breduced,np-3,ni,v,res,ier,istop,effet,ca,cb,dd,funcpajlongisplines)
              b=(/breduced,0.6324555d0,0.4427189d0,0.4516636d0/)
              method_GH=1
              call marq98J(k0,b,np,ni,v,res,ier,istop,effet,ca,cb,dd,funcpajlongisplines2)
            else 
              call marq98J(k0,b,np,ni,v,res,ier,istop,effet,ca,cb,dd,funcpajlongisplines2)
            end if 
          else 
      ...
      if(1.eq.0) then
        ...
        if (ni > 300) then !Au dela de 300 iteration la bar ne peut pas s'afficher (inpr limite a 255 caracteres),
          ! on fait le choix de laisser appara�tre les iteration malgre tout
    ...
    if(med(1).eq.1) then 
      if(ncenters.eq.0) then 
        !structure rt_nparams 
        !(1) = np 
        !(2) = nz
        !(3) = mt
        !(4) = nvarm 
        ...
        rt_sizes = (/param_res_rt(3),param_res_rt(2),param_res_rt(1),param_res_rt(4)/)
        rt_nparams = (/np,nz,mtaille(2),nva3,nva2,rank_trtM,rank_trtT,rank_trtMt,rank_time/)      
        call compute_rt(b,HIH,rt_nparams,zi,res_rt(:,1,1),rt_sizes,.FALSE.,res_rt)
      end if 
    end if 


    if(effet.eq.1.and.ier.eq.-1)then
      v((np-nva-indic_alpha-nparammed)*(np-nva-indic_alpha-nparammed+1)/2)=10.d10
    endif
    res01(effet+1)=res
    if (typeJoint.eq.2) then
      scaleweib(1) = 0.d0
    ...
    if (timedep.eq.0) then
      if(typeJoint.eq.2) then
        coefBetadc(1,:) = b((np-nva-nparammed+1):(np-nva-nparammed+nva2))
        coefBetaY(1,:) = b((np-nva-nparammed+nva2+1):(np-nva-nparammed+nva2+nva3))
        if(TwoPart.eq.1) coefBetaB(1,:) = b((np-nva-nparammed+nva2+nva3+1):(np-nva-nparammed+nva2+nva3+nvaB)) ! add TwoPart
        ...
        coefBeta(1,:) = b((np-nva-nparammed+1):(np-nva-nparammed+nva1))
        coefBetadc(1,:) = b((np-nva-nparammed+nva1+1):(np-nva-nparammed+nva1+nva2))
        coefBetaY(1,:) = b((np-nva-nparammed+nva1+nva2+1):np)
          ...
          do i=1,nva1
            ...
            if (filtretps(i).eq.1) then
              do k=-qorder+1,nbinnerknots
                coefBeta2 = coefBeta2 + b(np-(nva+npbetatps+nparammed)+p-1+k+qorder)*basis(k+qorder)
              end do
            else
              coefBeta2 = b(np-(nva+npbetatps+nparammed)+p)
            endif
      ...
      do j=1,ngtemp
        ...
        do i=1,nva2
          ...
          if (filtre2tps(i).eq.1) then
            do k=-qorder+1,nbinnerknots
              coefBeta2 = coefBeta2 + b(np-(nva+npbetatps+nparammed)+nva1+p-1+k+qorder)*basis(k+qorder)
            end do
          else
            coefBeta2 = b(np-(nva+npbetatps+nparammed)+nva1+p)
          endif
    ...
    deallocate(nmesB)
    if(mediation) deallocate(treat_ind,center_ind,matmc,matmc_frail)
    ...
    !deallocate(nmesrec,nmesrec1,nmesy,groupee,groupeey,nmes_o)
...
SUBROUTINE gauherJ21(ss,choix,nnodes)
  ...
  double precision,dimension(nnodes):: xx1,ww1
      ...
      else if(nnodes.eq.100) then 
              xx1(1:nnodes) = x100(1:nnodes)
              ww1(1:nnodes) = w100(1:nnodes)
      end if
...
double precision function func7J(frail2,frail)
        ...
        double precision,dimension(2):: uii,Xea!, Xea22
        double precision,dimension(nb1)::Xea22
...
double precision function func10J(frail3,frail2,frail)
        ...
        double precision,dimension(3):: uii,Xea!, Xea22,Xea
        double precision,dimension(nb1)::Xea22
        double precision,dimension(1)::uiiui
...
double precision function funcTP4J(frail4,frail3,frail2,frail)
        ...
        use tailles
        use comongroup,only:vet2!vet
        use comon,only:aux1,cdc,sigmae,&
            nva2,npp,nva3,vedc,nb1,betaD,etaD,t0dc,t1dc,etaydc,link,&
            vey, typeof,s_cag_id,s_cag,ut,utt,method_GH,b_lme,invBi_chol,&
            nbB, nby, nvaB, nmesB,TwoPart,veB!,nmesy add TwoPart
        use donnees_indiv
        IMPLICIT NONE
        ...
        double precision,dimension(4):: uii,Xea!, Xea22,Xea
        double precision,dimension(nb1)::Xea22
        double precision,dimension(1)::uiiui
...
double precision function func8J(frail,frail2)
        ...
        use comon,only:alpha,res1,res3,aux1,nig,cdc,sigmae,&
            ...
            t0dc,method_GH,b_lme,invBi_chol!effet,nb_re,nva,nmesy
        use donnees_indiv
...
double precision function func9J(frail,frail2,frail3)
        ...
        use comon,only:alpha,res1,res3,aux1,nig,cdc,sigmae,&
            ...
            ut,utt,t0dc,t1dc,method_GH,b_lme,invBi_chol!effet,nva,nmesy
        use donnees_indiv
            IMPLICIT NONE
    ...
    do j=1,nea
        ...
        do j=1,nea
            ...
            resultR = 0.d0
                    ...
                    res1(i) = res1(i) + resultR     !c'est d�j� res1-res3
...
double precision function funcG(frail5,frail4,frail3,frail2,frail)
            ...
            !    end if
            funcG=0.d0
                if ((method_GH.ne.3).and.(method_GH.ne.4)) then ! loglikelihood
                if(nb1.eq.1) then
                        if(link.eq.1) then
...
!! Quasi Monte-Carlo
subroutine QMC_JointModels(ss,func2,ndim)
  !use donnees,only:QMC500_1,QMC500_2 ! pour les points et poids de quadrature (fichier Adonnees.f90)
  use comon, only:ut!,nb1,nodes_number,ut
  use donnees_indiv
  use var_mediation,only:QMC1,QMC2,nmcfrail
  !use mpi
  !$ use OMP_LIB
  ...
  double precision,intent(out)::ss
  double precision::auxfunca !,mu1,vc1,ss2
  double precision,dimension(nmcfrail)::ss1
  double precision::x2222,somp !eps !ymarg contient le resultat de l'integrale
  double precision::func2
 ...
 select case(ndim)
   case(1)
      ii=0
       !print*,"here in QMC",nmcfrail,nsimu,QMC1(ii),ut(1,1)
       !!$OMP PARALLEL DO default(none) PRIVATE (ii,auxfunca) SHARED(nsimu,QMC5000_1,ss1)&
       !!$OMP SCHEDULE(Static)
          do ii=1,nsimu
              auxfunca=func2(0.d0,0.d0,0.d0,0.d0,ut(1,1)*QMC1(ii))
              ss1(ii)=auxfunca
          end do
       !!$OMP END PARALLEL DO
   case(2)
      ii=0
       !!$OMP PARALLEL DO default(none) PRIVATE (ii,auxfunca) SHARED(nsimu,QMC1000_1,QMC1000_2,ss1)&
       !!$OMP SCHEDULE(Static)     
          do ii=1,nsimu
             !print*,"here in QMC bf aux"
              xx = matmul(ut,(/QMC1(ii),QMC2(ii)/))
              auxfunca=func2(0.d0,0.d0,0.d0,xx(2),xx(1))
              ss1(ii)=auxfunca
              !print*,"here in QMC",nsimu,ii,ss1(ii)
          end do
       !!$OMP END PARALLEL DO
  end select
  ss=dble(SUM(ss1))/dble(nsimu)
  return
end subroutine QMC_JointModels
subroutine MCJointModels(ss,func2,ndim)
  use donnees,only:MC1,MC2,MC3,MC4,MC5,MC6,MC7,MC8,MC9,MC10 ! pour les points et poids de quadrature (fichier Adonnees.f90)
  use comon, only:ut!,nb1,nodes_number
  use donnees_indiv
  !use mpi
  !$ use OMP_LIB
  ...
  double precision,intent(out)::ss
  double precision::auxfunca !,mu1,vc1,ss2
  double precision,dimension(5000)::ss1
  double precision::x2222,somp !eps !ymarg contient le resultat de l'integrale
  double precision::func2
  double precision,dimension(2)::xx 
  double precision,dimension(5000)::MMC1,MMC2
 ...
 select case(ndim)
   case(1)
      ii=0
       !!$OMP PARALLEL DO default(none) PRIVATE (ii,auxfunca) SHARED(nsimu,MMC1,ss1)&
       !!$OMP SCHEDULE(Static)
          do ii=1,nsimu
              auxfunca=func2(0.d0,0.d0,0.d0,0.d0,ut(1,1)*MMC1(ii))
              ss1(ii)=auxfunca
          end do
       !!$OMP END PARALLEL DO
   case(2)
      ii=0
       !!$OMP PARALLEL DO default(none) PRIVATE (ii,auxfunca) SHARED(nsimu,MMC1,MMC2,ss1)&
       !!$OMP SCHEDULE(Static)     
          do ii=1,nsimu
             !print*,"here in QMC bf aux"
              xx = matmul(ut,(/MMC1(ii),MMC2(ii)/))
              auxfunca=func2(0.d0,0.d0,0.d0,xx(2),xx(1))
              ss1(ii)=auxfunca
              !print*,"here in QMC",nsimu,ii,ss1(ii)
          end do
...
!!!!
subroutine dummyMCOMP(int,numpatOMP,&
  nmescurOMP,ut2curOMP,muOMP,ycurrentOMP,it_curOMP,Z1OMP)
  use tailles
  use donnees
  use var_mediation,only:matmc_frail,nmcfrail
  use comon,only:nb1,maxmesy,ut!nby,auxig,typeof,nb1,nea
  !use donnees_indiv,only : frailpol,numpat
  !!$ use OMP_LIB 
  Implicit none
  ...
  !  double precision :: frail2
  integer::j
  !double precision,dimension(nodes_number):: xx1,ww1
  double precision,dimension(2)::xx 

  integer::numpatOMP,nmescurOMP
  double precision,dimension(nmescurOMP)::ycurrentOMP,muOMP
  double precision,dimension(maxmesy,nb1)::Z1omp
  double precision::ut2curOMP
  !double precision::frailpol 

  double precision,external::funcG2
  integer::it_curOMP

  int = 0.d0 
  do j=1,nmcfrail
    xx = matmul(ut,matmc_frail(j,:))
    int= int +  funcG2(xx(2),xx(1),numpatOMP,nmescurOMP,ut2curOMP,muOMP,ycurrentOMP,Z1OMP,it_curOMP)
  end do 
  int=int/dble(nmcfrail)
end subroutine 
SUBROUTINE gauherJ22OMP(ss,choix,nnodes,numpatOMP,nmescur,ut2cur,&
                        mu,ycurrent,it_cur,Z1omp)
  use tailles
  use donnees
  use comon,only:method_GH,invBi_cholDet,nea,typeJoint,nb1,maxmesy!nby,auxig,typeof,nb1,nea
  !use donnees_indiv,only : frailpol,numpat
  !!$ use OMP_LIB
  Implicit none
  double precision,intent(out)::ss
  ...
  integer::numpatOMP,nmescur
  double precision,dimension(nmescur)::ycurrent,mu
  double precision,dimension(maxmesy,nb1)::Z1omp
  double precision::ut2cur
  double precision::frailpol
  integer::it_cur
  ...
  if(method_GH.eq.0) then
      ...
      do j=1,nnodes
        ...
        frailpol = xx1(j)
        call gauherJ21OMP(auxfunca,choix,nnodes,numpatOMP,nmescur,ut2cur,&
                                mu,ycurrent,it_cur,Z1omp,frailpol)
        ss = ss+ww1(j)*(auxfunca)
    ...
    if(typeJoint.eq.2.and.nea.eq.2)ss = ss*invBi_cholDet(numpatOMP) *2.d0**(nea/2.d0)
  end if
  return
END SUBROUTINE gauherJ22OMP
SUBROUTINE gauherJ21OMP(ss,choix,nnodes,numpat,nmescur,ut2cur,&
                        mu,ycurrent,it_cur,Z1,frailpol)
  use tailles
  use donnees
  use comon,only:nb1,typeJoint,nea,method_GH,invBi_cholDet,nb1,maxmesy!auxig,typeof,nby
  !use donnees_indiv,only : frailpol,frailpol2,frailpol3,numpat
  Implicit none
  double precision,intent(out)::ss
  double precision,intent(in)::frailpol
  integer,intent(in)::choix,nnodes
  double precision::auxfunca !func8J,func9J,func10J,func11J, funcTP4J, add TwoPart
  double precision,external::funcG2 ! add TwoPart
  integer::j
  double precision,dimension(nnodes):: xx1,ww1

  integer::numpat,nmescur,it_cur
  double precision,dimension(nmescur)::ycurrent,mu
  double precision,dimension(maxmesy,nb1)::Z1
  double precision::ut2cur
  ...
  if(method_GH.eq.0) then
    do j=1,nnodes
      if (choix.eq.3) then
        if(typeJoint.eq.2.and.nb1.eq.2) then
          auxfunca = funcG2(frailpol,xx1(j),numpat,nmescur,ut2cur,mu,ycurrent,Z1,it_cur)
        endif
        ss = ss+ww1(j)*(auxfunca)
      endif
    end do
    ...
    if(typeJoint.eq.2.and.nea.eq.1)ss = ss*invBi_cholDet(numpat)*2.d0**(1.0d0/2.d0)
  end if
  return
END SUBROUTINE gauherJ21OMP
double precision function funcG2(frail2,frail,numpat,nmescur,ut2cur,mu,ycurrent,Z1,it_cur)
  use tailles
  !use comongroup,only:vet2!vet
  ...
  use comon,only:cdc,sigmae,nmesy,&
  nva2,npp,nva3,vedc,nb1,betaD,etaD,t0dc,t1dc,etaydc,link,&
  vey, typeof,s_cag_id,ut,utt,method_GH,b_lme,invBi_chol,&
  nby, nvaB, nmesB,TwoPart,numInter,positionVarT,nb_re,nva,&
  maxmesy,effet,indic_alpha,netadc,netar!,zi,nbb,numinterb,s_cag,veb
  !auxig,alpha,sig2,res1,res3,nb1,nea,nig,utt,
  use donnees_indiv,only:GLMloglink0,MTP0,b1
  use var_surrogate,only:mediation
  use var_mediation,only:nuzm,nuzt,treat_ind,nparammed,&
                         betaztime
  IMPLICIT NONE
  integer::numpat,nmescur
  double precision,dimension(nmescur,1)::mu
  double precision,dimension(nmescur)::ycurrent
  double precision,dimension(maxmesy,nb1)::Z1
  double precision::ut2cur
  double precision,intent(in)::frail,frail2
  double precision, dimension(numpat)::auxG
  ...
  double precision,dimension(nb1,nb1)::mat,matb_chol
  double precision, dimension(1)::current_meanG
  double precision :: yscalar,yscalarlog,eps,finddet,det,prod_cag,yscal1,yscal2!,alnorm
  integer :: j,jj,i,k,ier
  logical :: upper
  double precision,external::survdcCMOMP
  double precision :: resultdc,abserr,resabs,resasc,vet2
  double precision,parameter::pi=3.141592653589793d0
  double precision :: resultf1, resultf2, f1, f2, logNormCum ! add TwoPart
  double precision,dimension(1) :: Bscalar,Bscal,Bcurrentvalue, cmY,cmGtemp!Bcv
  integer :: counter, counter2 ! add for current-level interaction
  !double precision::resint
  integer::it_cur,n
  double precision,dimension(-2:npp)::the
  !double precision::lim=1d-9

  n = (npp-nva-effet-indic_ALPHA-1-nb_re-netadc-netar-nparammed)/(effet+1)
  ...
  if(nva2.gt.0)then
    vet2 = 0.d0
    do j=1,nva2
      vet2 =vet2 + b1(npp-nva3-nva2-nvaB-nparammed+j)*dble(vedc(i,j))
    end do
    vet2 = dexp(vet2)
  else
    vet2=1.d0
  ...
  if(mediation)then
    vet2 = vet2*dexp((nuzt)*treat_ind(i))
  end if
  if(link.eq.1) then !******** Random Effects **********************
    ! pour le calcul des integrales / pour la survie, pas les donno?=es recurrentes:
    if(typeof.eq.2) then
    ...
    !end if
  else if(link.eq.4) then ! current slope
      auxG(i) = ut2cur*vet2
  else if(link.eq.2) then !current level
    counter=0
    counter2=1
    if((nva3-1).gt.0) then ! set the value of covariates at time to event! (interaction must be computed accordingly)
      x2curG(1,1) = 1.d0
    ...
    if(nb1.eq.1) then
      current_meanG(1) =dot_product(x2curG(1,1:nva3),b1((npp-nva3-nparammed+1):(npp-nparammed)))+z1YcurG(1,1)*Xea(1)
      if(mediation) then
        !current_meanG(1) = current_meanG(1)  + nuzm*treat_ind(i)
      end if
    else if(nb1.gt.1) then
      if(TwoPart.eq.1) then
        if(nb1.eq.2) then
        ...
        !  fixed_Binary=0.d0
        !end if
        !Bcv=fixed_Binary+MATMUL(X2BcurG,b1((npp-nvaB-nparammed+1):(npp-nparammed)))+Matmul(z1BcurG,Xea22)
        !Bcurrentvalue=dexp(Bcv)/(1+dexp(Bcv))
        !cmY=0.d0
        !cmY = (MATMUL(x2curG,b1((npp-nva3-nvaB-nparammed+1):(npp-nvaB-nparammed)))+Matmul(z1YcurG,Xea22))
        !if(GLMloglink0.eq.0) then
        !  current_meanG = cmY*Bcurrentvalue
        !else if(GLMloglink0.eq.1) then
        !  if(MTP0.eq.0) then
        ...
        if(GLMloglink0.eq.0) then
            current_meanG = MATMUL(X2curG,b1((npp-nva3-nparammed+1):(npp-nparammed)))+Matmul(z1YcurG,Xea22)
            if(mediation) then
              current_meanG(1) = current_meanG(1)  + (nuzm)*treat_ind(i)
            end if
        else if(GLMloglink0.eq.1) then
          cmY = MATMUL(X2curG,b1((npp-nva3-nparammed+1):(npp-nparammed)))+Matmul(z1YcurG,Xea22)
          current_meanG = dexp(cmY)
        end if
      end if
    end if
    !if(timetype.eq.1) then
      !call integralcl(t1dc(i)-lim,the,zi,n,etaydc(1)*(b1(18)+Xea22(2)+betaztime*treat_ind(i)),resint)
      !auxG(i) = resint*vet2*dexp(etaydc(1)*(current_meanG(1)-t1dc(i)*(b1(18)+Xea22(2)+betaztime*treat_ind(i))))
    !else
    call integrationdcOMP(survdcCMOMP,t0dc(i),t1dc(i),resultdc,abserr,resabs,resasc,i,it_cur,b1,npp,xea22)
    auxG(i) = resultdc
    !end if
  end if
  yscalar = 0.d0
  ...
  if(nmescur.gt.0) then
    if(s_cag_id.ne.1) then ! no left censoring
      do k=1,nmescur
        if(GLMloglink0.eq.0) then ! original scale (no transformation of the longi outcome)
          yscalar = yscalar + (ycurrent(k)-mu1G(k,1))**2
        else if(GLMloglink0.eq.1) then ! lognormal density
  ...
  if (method_GH.ne.3) then ! loglikelihood
    if(nb1.eq.1) then
      if(link.eq.1) then
        funcG2 =   dlog(prod_cag)-(yscalar**2.d0)/(2.d0*sigmae)+yscalarlog& !longi part
        - (Xea(1)**2.d0)/(2.d0*ut(1,1)**2) & ! random effects density
        - dlog(ut(1,1))-dlog(2.d0*pi)/2.d0& ! random effects density
        -auxG(i)*dexp(etaydc(1)*Xea(1))  + cdc(i)*etaydc(1)*Xea(1) ! survival part
      else
        funcG2 =   dlog(prod_cag)-(yscalar**2.d0)/(2.d0*sigmae)+yscalarlog&
        - (Xea(1)**2.d0)/(2.d0*ut(1,1)**2)&
        - dlog(ut(1,1))-dlog(2.d0*pi)/2.d0&
        -auxG(i) &
        + cdc(i)*etaydc(1)*current_meanG(1)
      ...
      if(link.eq.1) then
        ...
        -dlog(2.d0*pi)&
        +Bscalar(1)& ! add TwoPart
        -auxG(i)*dexp(etaydc(1)*(b1(18)+Xea22(2)+betaztime*treat_ind(i)))&
        + cdc(i)*(etaydc(1)*(b1(18)+Xea22(2)+betaztime*treat_ind(i)))
      end if
    else
      if(link.eq.1) then
        funcG2 = dlog(prod_cag)-(yscalar**2.d0)/(2.d0*sigmae)+yscalarlog&
...
subroutine integralcl(t1dc,the,zi,n,alpha,xx)
  use tailles,only:npmax
  implicit none
  double precision,intent(out)::xx
  double precision::t1dc
  double precision,dimension(-2:npmax)::the,zi
  double precision::som
  double precision::FF,FF1,FF2,FF3
  double precision::alpha
  external::FF,FF1,FF2,FF3
  integer::j,k,n
  double precision::lim=1d-9
  double precision::res,res1,res2,res3
  som=0.0d0
  do k = 2,n-1
    if ((t1dc.ge.zi(k-1)).and.(t1dc.lt.zi(k)))then
        j = k-1
    end if
  end do

  do k=1,j-1
    res = FF(zi(k+1)-lim,alpha,n,zi)-FF(zi(k),alpha,n,zi)
    res1 = FF1(zi(k+1)-lim,alpha,n,zi)-FF1(zi(k),alpha,n,zi)
    res2 = FF2(zi(k+1)-lim,alpha,n,zi)-FF2(zi(k),alpha,n,zi)
    res3 = FF3(zi(k+1)-lim,alpha,n,zi)-FF3(zi(k),alpha,n,zi)
    som = som +the(k)*res+the(k-1)*res1+the(k-2)*res2+the(k-3)*res3
  end do
  res = FF(t1dc,alpha,n,zi)-FF(zi(j),alpha,n,zi)
  res1 = FF1(t1dc,alpha,n,zi)-FF1(zi(j),alpha,n,zi)
  res2 = FF2(t1dc,alpha,n,zi)-FF2(zi(j),alpha,n,zi)
  res3 = FF3(t1dc,alpha,n,zi)-FF3(zi(j),alpha,n,zi)
  som = som +the(j)*res+the(j-1)*res1+the(j-2)*res2+the(j-3)*res3
  xx=som
end subroutine
double precision function FF(x,alpha,n,zi)
  use tailles,only:npmax
  implicit none
  integer::j,k,n
  ...
  double precision::zk,zkm1,zkm2,zkm3,zkm4,zkp1,zkp2,zkp3
  double precision::c1!,c2,c3
  double precision::a,b,c,d
  double precision,dimension(4)::coeffs!,coeff
  j=0
  do k = 2,n-1
  ...
  call coeff(alpha,coeffs)
  FF=dexp(x*alpha)*(coeffs(1)*x**3+coeffs(2)*x**2+coeffs(3)*x+coeffs(4))
  return
end function
double precision function FF1(x,alpha,n,zi)
  use tailles,only:npmax
  implicit none
  integer::j,k,n
  double precision::h,hh,h2,h3,h4,h3m,h2n,hn,hh3,hh2
  ...
  c1 = (4.0d0/(h3m*h2n*hh*h))
  c2 = -(4.0d0/(h3m*h2*h*h2n))
  c3 = 4.0d0/(h3m*h3*h2*h)

  a = - c1 + c2 - c3
  b = c1*(zk+2.0d0*zkm2)+c2*(-zkp1-zkm1-zkm2)+c3*(zkp2+2d0*zkm1)
  c = c1*(-2d0*zkm2*zk-zkm2**2)+c2*(zkp1*(zkm1+zkm2)+zkm1*zkm2)+c3*(-2.d0*zkm1*zkp2-zkm1**2)
  d = c1*((zkm2**2)*zk)+c2*(-zkp1*zkm1*zkm2)+c3*((zkm1**2)*zkp2)
  coeffs=(/a,b,c,d/)
  call coeff(alpha,coeffs)
  FF1=dexp(x*alpha)*(coeffs(1)*x**3+coeffs(2)*x**2+coeffs(3)*x+coeffs(4))

  return
end function
double precision function FF2(x,alpha,n,zi)
  use tailles,only:npmax
  implicit none
  integer::j,k,n!,i
  double precision::h,hh,h2,h3,h4,h3m,h2n,hn,hh3,hh2
  ...
  c1 = (4.d0/(hh2*hh*h*hn))
  c2 = -(4.d0/(hh2*h2n*hh*h))
  c3 = 4.d0/(hh2*h2*h*h2n)

  a = c1 - c2 + c3
  b = c1*(-2*zk-zkm3)+c2*(zk+zkm2+zkp1)+c3*(-zkm1-2*zkp1)
  c = c1*(zk**2+2*zk*zkm3)-c2*(zk*(zkm2+zkp1)+zkp1*zkm2)+c3*(2*zkp1*zkm1+zkp1**2)
  d = c1*(-zkm3*zk*zk)+c2*(zkp1*zkm2*zk)+c3*(-zkm1*zkp1*zkp1)
  coeffs=(/a,b,c,d/)
  call coeff(alpha,coeffs)
  FF2=dexp(x*alpha)*(coeffs(1)*x**3+coeffs(2)*x**2+coeffs(3)*x+coeffs(4))
  return
end function
double precision function FF3(x,alpha,n,zi)
  use tailles,only:npmax
  implicit none
  integer::j,k,n
  double precision::h,hh,h2,h3,h4,h3m,h2n,hn,hh3,hh2
  ...
  coeffs=(/a,b,c,d/)
  call coeff(alpha,coeffs)
  FF3=dexp(x*alpha)*(coeffs(1)*x**3+coeffs(2)*x**2+coeffs(3)*x+coeffs(4))
  return
end function
subroutine coeff(alpha,coeffs)
  ! This function returns the coefficients of the third degree polynomial
  ! Q(x) in the function F(x) = exp(alpha*x)*Q(x) which is a primitive
  ! of the function f(x) = exp(alpha*x)*P(x) where P(x) is a third dregree
  ! polynomial whose coefficients are given in coeffs.
  ! coeff(1) is the coefficient related to x³ and so on, coeff(4) being the constant.
  implicit none
  double precision,intent(in)::alpha
  double precision,dimension(4),intent(out)::coeffs
  double precision::a,b,c,d
  a=coeffs(1)/alpha
  b=(1.0d0/alpha)*(coeffs(2)-3.0d0*a)
  c=(1.0d0/alpha)*(coeffs(3)-2.0d0*b)
  d=(1.0d0/alpha)*(coeffs(4)-c)
  coeffs=(/a,b,c,d/)
  return
end subroutine
double precision function survdcCMOMP(tps,i,it_curOMP,bh,np,frail)
  use tailles
  use comon
  use betatttps
  use donnees_indiv
  ...
  if(nva2.gt.0)then
    vet2 = 0.d0
    do j=1,nva2
      vet2 =vet2 + bh(np-nva3-nva2-nvaB-nparammed+j)*dble(vedc(i,j))
    end do
    vet2 = dexp(vet2)
  else
    vet2=1.d0
  ...
  if((nva3-1).gt.0) then ! set the value of covariates at time to event! (interaction must be computed accordingly)
    x2curG(1,1) = 1.d0
    do k=2,nva3
      x2curG(1,k) = dble(vey(it_curOMP+1,k))
    end do
    resultf1=f1(tps) ! maybe remove this calculus if not needed (non-linear slopes)
    resultf2=f2(tps)
    if(numInter.ge.1)then
      do counter = 1,numInter !in case of multiple interactions
        if(positionVarT(counter2+3).eq.0) then ! linear
          ...
          if(positionVarT(counter2+2).ne.0) then
            if(positionVarT(counter2).le.100) then ! if interaction terms not included
              x2curG(1,positionVarT(counter2+2)) =tps*dble(vey(it_curOMP+1,positionVarT(counter2)))
            else if(positionVarT(counter2).gt.100) then
              x2curG(1,positionVarT(counter2+2)) =tps*dble(vedc(i,positionVarT(counter2)-100))
            end if
          end if
          ...
          if(positionVarT(counter2+2).ne.0) then
            if(positionVarT(counter2).le.100) then ! if interaction terms not included
              x2curG(1,positionVarT(counter2+2)) =resultf1*dble(vey(it_curOMP+1,positionVarT(counter2)))
            else if(positionVarT(counter2).gt.100) then
              x2curG(1,positionVarT(counter2+2)) =resultf1*dble(vedc(i,positionVarT(counter2)-100))
            end if
          end if
          ...
          if(positionVarT(counter2+2).ne.0) then
            if(positionVarT(counter2).le.100) then ! if interaction terms not included
              x2curG(1,positionVarT(counter2+2)) =resultf2*dble(vey(it_curOMP+1,positionVarT(counter2)))
            else if(positionVarT(counter2).gt.100) then
              x2curG(1,positionVarT(counter2+2)) =resultf2*dble(vedc(i,positionVarT(counter2)-100))
            end if
          end if
  ...
  if(TwoPart.eq.1) then
    ...
    Bcv=fixed_Binary+dot_product(X2BcurG(1,1:nvaB),bh((np-nvaB-nparammed+1):(np-nparammed)))&
                    +dot_product(z1BcurG(1,1:nb1),frail(1:nb1))
    Bcurrentvalue=dexp(Bcv)/(1+dexp(Bcv))
    cmY = (dot_product(x2curG(1,1:nva3),bh((np-nva3-nvaB-nparammed+1):(np-nvaB-nparammed)))&
           +dot_product(z1curG(1, 1:nb1),frail(1:nb1)))
    if(GLMloglink0.eq.0) then
      current_meanG = cmY*Bcurrentvalue
    else if(GLMloglink0.eq.1) then
    ...
    if(GLMloglink0.eq.0) then
      if(nea.gt.1) then
        current_meanG =dot_product(x2curG(1,1:nva3),bh((np-nva3-nparammed+1):(np-nparammed)))&
        +dot_product(z1curG(1,1:nb1),frail(1:nb1))
        if(mediation) then
          current_meanG(1) = current_meanG(1) + (nuzm)*treat_ind(i)
        end if
      else
        current_meanG = dot_product(x2curG(1,1:nva3),bh((np-nva3+1-nparammed):(np-nparammed)))&
                        +z1curG(1,1:nb1)*frail(1:nb1)
      end if
    else  if(GLMloglink0.eq.1) then
      if(nea.gt.1) then
        cmY =dot_product(x2curG(1,1:nva3),bh((np-nva3-nparammed+1):(np-nparammed)))&
        +dot_product(z1curG(1,1:nb1),frail(1:nb1))
        current_meanG=dexp(cmY)
      else
        cmY = dot_product(x2curG(1,1:nva3),bh((np-nva3-nparammed+1):(np-nparammed)))&
  ...
  case(0) ! calcul du risque splines
    if(netar+netadc.ge.1) then
      ...
      n = (np-nva-effet-indic_ALPHA-nb_re - netadc - netar-nparammed)/(effet+1)
    endif
    do k=1,n
      if(typeJoint.eq.2)the2(k-3)=(bh(k))**2.d0
      if(typeJoint.eq.3)the2(k-3)=(bh(k+n))**2.d0
  ...
  if(res_ind.eq.1) bbb = Rdc_res(i)
  if(link.eq.2) then
    survdcCMOMP =bbb*vet2*dexp(etaydc(1)*current_meanG(1))!+cdc(i)*etaydc1*current_meanG(1)
  else if(link.eq.3) then
    survdcCMOMP =bbb*vet2*dexp(etaydc(1)*Bcurrentvalue(1)+etaydc(2)*cmY(1))!+cdc(i)*etaydc1*current_meanG(1)
  end if
  return
end function survdcCMOMP
